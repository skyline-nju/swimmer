...
template <typename TPar>
LatUniDomain_2::LatUniDomain_2(const cmdline::parser& cmd,  // NOLINT
                               std::vector<TPar>& p_arr)
...
template <typename TPar>
void LatUniDomain_2::run(const cmdline::parser& cmd, std::vector<TPar*>& ptr_arr) {
  const auto n_step = cmd.get<unsigned int>("n_step");
  const auto alpha = cmd.get<double>("alpha");
  const auto max_capacity = cmd.get<int>("n_max");
...
template <typename TPar>
void LatUniDomain_2::run_and_tumble_1(TPar& p, double alpha, int max_capacity) {
  if (p.uy) {
    auto y_new = p.y + p.uy;
