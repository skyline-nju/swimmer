...
public:
  ...
  void move (TPar &p, Ran &myran, UniFunc tangle) const;

  template<typename TPar, typename UniFunc>
  void operator() (TPar &p, Ran &myran, UniFunc tangle) const {
    move(p, myran, tangle);
  }
  template <typename TPar, typename UniF1, typename UniF2>
  void operator() (TPar &p, Ran &myran, UniF1 tangle, UniF2 wall_force) const {
    wall_force(p);
    move(p, myran, tangle);
  }
private:
...
template <typename TPar, typename UniFunc>
void Run_and_tumble::move (TPar& p, Ran& myran, UniFunc tangle) const {
