...
WetDataExporter::WetDataExporter(const cmdline::parser & cmd):
  ...
  check_err(stat, __LINE__, __FILE__);
  stat = nc_def_dim(ncid_, "left and right sides", 2, &left_right_dim);
...
void WetDataExporter::dump_frame(int i_step,
  ...
  { // wetting fraction
    ...
    auto stat = nc_put_vara(ncid_, wetting_frac_id_, startset, countset,
                            &packing_frac[0]);
    check_err(stat, __LINE__, __FILE__);
  }
  { // thickness and particle number profile
    ...
    auto stat = nc_put_vara(ncid_, thickness_profile_id_, startset, countset,
                            &thickness_profile[0]);
    check_err(stat, __LINE__, __FILE__);
    stat = nc_put_vara(ncid_, num_profile_id_, startset, countset,
                            &num_profile[0]);
    check_err(stat, __LINE__, __FILE__);
  }
...
void WetDataExporter::set_chunk_and_deflate() {
  { // time steps
